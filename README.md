...
3. Push to GitHub – the existing `newsletter.yml` workflow will continue sending emails; the new `weekly-report.yml` workflow will regenerate and publish static HTML every Monday.
   ...
   *Emails generated by `newsletter.yml` no longer link to a single dated file. Instead, they point at the `index.html` page that the weekly workflow publishes; that page always exists and contains links to the current and past reports.  This prevents 404s when a newsletter is sent before the report has been deployed.*
